...
POST /api/vector/rebuild       增量同步向量库（基于时间戳）
POST /api/vector/sync          增量同步向量库（基于时间戳）
POST /api/vector/full-rebuild  全量重建向量库 + 清理孤儿数据
"""
...
def full_rebuild_vectors(
    ...
    示例：POST /api/vector/full-rebuild?resource_type=1

    适用场景：metadata 结构变更（如新增 tags 字段）、数据修复
    ...
    返回：
    {
        ...
        "failed": 0,
        "orphans_deleted": 3,
        "message": "全量重建完成：成功 1472 条，失败 0 条，清理孤儿 3 条"
    }
    """
    try:
        rt = ResourceType(resource_type)
    ...
    result = rebuild_all_vectors_by_type(db, rt, source_id=source_id)
    result["resource_type"] = rt.name
    return result
@router.post("/sync")
def sync_vectors(
