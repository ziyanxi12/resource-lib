...
class VectorSearchLog(Base):
    ...
    result_count  = Column(Integer, nullable=True, comment="命中总数（各组求和，含重复）")
    result_ids    = Column(JSON, nullable=True, comment="命中资源ID扁平去重列表")
    top_score     = Column(Float, nullable=True, comment="最高相关性分数")
    results       = Column(JSON, nullable=True, comment="完整命中结果二维数组（全量，按 response_mode 原样存）")
