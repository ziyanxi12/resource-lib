...
def create_account(
    ...
    if not nick:
        nick = _lookup_nick_name(account)
...
def _lookup_nick_name(account: str) -> Optional[str]:
    """调 USER_INFO_API_URL 获取昵称，查不到返回 None（不报错，不阻塞）。"""
    try:
        from app.services.user_service import _fetch_user_from_api
        info = _fetch_user_from_api(account)
        return (info.get("nickName") or None) if info else None
    except Exception:
        return None
